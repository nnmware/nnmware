...
def amp(text):
    ...
    return mark_safe(amp_finder.sub(r"""\1<span class="amp">&amp;</span>\3""", text))
...
def caps(text):
    ...
    """
    try:
        import nnmware.core.smartypants
    except ImportError:
        if settings.DEBUG:
...
def initial_quotes(text):
    ...
    return mark_safe(quote_finder.sub(_quote_wrapper, text))
...
def smartypants(text):
    ...
    try:
        ...
        return mark_safe(smartypants.smartyPants(text))
...
def widont(text):
    ...
    widont_finder = re.compile(r"""(\s+)                                # the space to replace
                                   ...
                                   """, re.VERBOSE)
    return mark_safe(widont_finder.sub(r'&nbsp;\2', text))


def mdash(text):
    mdash_finder = re.compile(r"([^\.]\s)\-(\s[^\.])")
    return mark_safe(mdash_finder.sub(r'\1&mdash;\2', text))
