...
class AbstractEmployee(AbstractImg):
    ...
    def get_agent_avatar(self):
        if self.agent_img is not None and self.agent_img != '':
...
class Company(AbstractName, AbstractLocation, MetaGeo, AbstractWTime, AbstractDate, AbstractTeaser):
    ...
    fullname = models.CharField(verbose_name=_("Full Name"), max_length=255, db_index=True, blank=True)
    fullname_en = models.CharField(verbose_name=_('Full Name(English)'), max_length=255, blank=True,
                                   db_index=True)
