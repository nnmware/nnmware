...
class ReportView(CurrentUserSuperuser, ListView):
    ...
    def get_queryset(self):
        ...
        if report_type == 'all':
            ...
            nullroom = Room.objects.filter(Q(availability__date__range=(datetime.now(),
                datetime.now() + timedelta(days=13))) | Q(availability__placecount=0)).annotate(num_days=Count('pk')).\
