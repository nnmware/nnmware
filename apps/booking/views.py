...
class HotelList(RedirectHttpView, ListView):
    ...
    def get_queryset(self):
        ...
        if 1>0: #self.request.is_ajax():
            ...
            if 1>0: #not data_key:
                if self.city:
                    search_hotel = Hotel.objects.select_related('address', 'review').filter(city=self.city)  # .exclude(payment_method=None)
                else:
                    search_hotel = Hotel.objects.select_related('address', 'review').all()
                if searched_date:
                    result = []
