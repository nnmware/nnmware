        ...
        if 1>0: #self.request.is_ajax():
            ...
            if 1>0: #not data_key:
                ...
                if searched_date:
                    ...
                    avail_room = [x[0] for x in Availability.objects.filter(room__pk__in=rooms_list, date__in=date_gen,
                        min_days__lte=need_days).order_by('room').annotate(num_days=Count('room')).\
                        filter(num_days__gte=need_days).values_list('room__pk', 'num_days')]
                    # avail_room = []
                    # for item in avail:
                    #     if item[1] >= need_days:
