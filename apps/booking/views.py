...
class HotelAjaxList(AjaxViewMixin, RedirectHttpView, ListView):
    paginate_by = 20
    model = Hotel
    template_name = "hotels/list.html"
    search = 0
    ...
    def get_queryset(self):
        ...
        if 1 > 0: #self.request.is_ajax():
            if self.request.is_ajax():
                self.template_name = "hotels/list.html"
...
class ReportView(CurrentUserSuperuser, ListView):
    ...
    def get_queryset(self):
        ...
        if report_type == 'all':
            ...
            result = Hotel.objects.select_related().filter(agentpercent__date__lte=datetime.now()).annotate(Max('agentpercent__date')).\
