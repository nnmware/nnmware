...
#        ordering = ['category__name','order_in_list','name']
    ...
    def allorders(self):
        ...
        return Order.objects.active().dates('created_date', 'day')
        #.filter(pk__in=allitems).extra({'date_created' : "date(created_date)"}).values('date_created').annotate(orders=Count('id'))
